...
async function searchUser() {
  ...
  if (!username) return alert("𝐈𝐥𝐭𝐢𝐦𝐨𝐬 𝐔𝐬𝐞𝐫 𝐊𝐢𝐫𝐢𝐭𝐢𝐧𝐠");
  ...
  try {
    ...
    if (!response.ok) throw new Error("𝐔𝐬𝐞𝐫 𝐓𝐨𝐩𝐢𝐥𝐦𝐚𝐝𝐢");
...
async function fetchRepositories(reposUrl) {
  reposContainer.innerHTML = '<div class="loading-repos">𝐑𝐞𝐩𝐨𝐬𝐭𝐨𝐫𝐲 𝐀𝐧𝐢𝐪𝐥𝐚𝐧𝐦𝐨𝐪𝐝𝐚.....</div>';
...
function displayRepos(repos) {
  if (repos.length === 0) {
    reposContainer.innerHTML = '<div class="no-repos">𝐑𝐞𝐩𝐨𝐬𝐭𝐨𝐫𝐲 𝐓𝐨𝐩𝐢𝐥𝐦𝐚𝐝𝐢</div>';
    return;
  }
  ...
  repos.forEach((repo) => {
    ...
    repoCard.innerHTML = `
      ...
      <p class="repo-description">${repo.description || "𝐓𝐚𝐫𝐢𝐟 𝐌𝐚𝐯𝐣𝐮𝐝 𝐄𝐦𝐚𝐬"}</p>
      <div class="repo-meta">
        ${
          repo.language
            ? `
...
function displayUserData(user) {
  ...
  bioElement.textContent = user.bio || "𝐁𝐢𝐨 𝐌𝐚𝐯𝐣𝐮𝐝 𝐄𝐦𝐚𝐬";

  locationElement.textContent = user.location || "𝐉𝐨𝐲𝐥𝐚𝐬𝐡𝐦𝐚𝐠𝐚𝐧";
  ...
  if (user.twitter_username) {
    ...
    twitterElement.textContent = "𝐓𝐰𝐢𝐭𝐭𝐞𝐫 𝐘𝐨❜𝐪";
    twitterElement.href = "#";
  }
